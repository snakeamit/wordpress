<?php
  function addLiveCurrencyRates($url, $idd) {
    ...
    $high = 0; //$result[0]->{'high'}; //$result->{'currency'}[0]->{'daily_highest'}; //$result[0]->{'high'}; 

    
    $low =0; // $result[0]->{'low'}; //$result->{'currency'}[0]->{'daily_lowest'}; //$result[0]->{'low'};
    ...
    if($rate){
$sql="UPDATE `live-rates-ibr` SET `rate`=$rate,`bid`=$bid,`ask`=$ask,`high`=$high,`low`=$low,`open`=$open,`close`=$close,`timestamp`='$timestamp' WHERE `id`=7";
  ...
  addLiveCurrencyRates("https://www.live-rates.com/api/price?rate=USD_THB&key=","7");
