<?php
  function addLiveCurrencyRates($url, $idd) {
    ...
    if($rate){
$sql="UPDATE `live-rates-ibr` SET `rate`=$rate,`bid`=$bid,`ask`=$ask,`high`=$high,`low`=$low,`open`=$open,`close`=$close,`timestamp`='$timestamp' WHERE `id`=5";
  ...
  addLiveCurrencyRates("https://www.live-rates.com/api/price?rate=USD_CAD&key=","5");
