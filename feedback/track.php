...
<head>
	...
	<link rel="shortcut icon" href="https://learn.ibrlive.com/favicon.ico">
...
<body style="font-size: 14px;">
	...
	if(isset($_REQUEST['userphone2']))
	    $_SESSION['userphone2']=$_REQUEST['userphone2'];
	    
	if(isset($_REQUEST['usercity']))
	...
	if(isset($_REQUEST['userstatus']))
	    $_SESSION['userstatus']=$_REQUEST['userstatus'];    
	    
	if(isset($_REQUEST['userdt']))
	    $_SESSION['userdt']=$_REQUEST['userdt'];
	    
	if(isset($_SESSION['username']) and $_SESSION['username']!=""){
		$condition	.=	' AND username LIKE "%'.$_SESSION['username'].'%" ';
	}
	if(isset($_SESSION['person']) and $_SESSION['person']!=""){
		$condition	.=	' AND person LIKE "%'.$_SESSION['person'].'%" ';
	}
	if(isset($_SESSION['useremail']) and $_SESSION['useremail']!=""){
		$condition	.=	' AND useremail LIKE "%'.$_SESSION['useremail'].'%" ';
	}
	if(isset($_SESSION['userphone']) and $_SESSION['userphone']!=""){
		$condition	.=	' AND userphone LIKE "%'.$_SESSION['userphone'].'%" ';
		$condition	.=	' OR userphone2 LIKE "%'.$_SESSION['userphone'].'%" ';
	}
	#if(isset($_REQUEST['userphone']) and $_REQUEST['userphone']!=""){
	#	$condition	.=	' AND userphone2 LIKE "%'.$_REQUEST['userphone'].'%" ';
	#}
	if(isset($_SESSION['usercity']) and $_SESSION['usercity']!=""){
		$condition	.=	' AND usercity LIKE "%'.$_SESSION['usercity'].'%" ';
	}
	if(isset($_SESSION['userstate']) and $_SESSION['userstate']!=""){
		$condition	.=	' AND userstate LIKE "%'.$_SESSION['userstate'].'%" ';
	}
	if(isset($_SESSION['userstatus']) and $_SESSION['userstatus']!=""){
		$condition	.=	' AND userstatus LIKE "%'.$_SESSION['userstatus'].'%" ';
	}

	if(isset($_SESSION['userdt']) and $_SESSION['userdt']!=""){

		$condition	.=	' AND DATE(userdt)="'.$_SESSION['userdt'].'" ';

	}

    if($condition != ''){
	$userData	=	$db->getAllRecords('users','*',$condition,'ORDER BY id ASC LIMIT 8000');
    }
	?>
							...
							<div class="col-sm-2">
								<div class="form-group">
									...
									<select name="userstatus" id="userstatus" class="form-control">
									    ...
									    <option <?php if($_REQUEST['userstatus']=="Invalid") echo "selected";?> value="Invalid">Invalid</option>
									</select>    
								</div>
							</div>
			...
			<table id="exporter" name="exporter" class="table table-striped table-bordered" style="height: 500px;">
				...
				<tbody>
					...
					<tr>
						...
						<td>
						<?php echo isset($val['userdt'])? date("d-m-Y", strtotime($val['userdt'])) :''
						?>
						</td>
						
						<td><?php echo $val['userremarks'];?></td>
						<td><?php echo $val['userstatus'];?></td>
						<td align="center">
						    <a href="add-info.php?infoId=<?php echo $val['id'];?> &infoName=<?php echo $val['username'];?> &infoPerson=<?php echo $val['person'];?> &infoPhone1=<?php echo $val['userphone'];?> &infoPhone2=<?php echo $val['userphone2'];?>" class="text-primary"><i class="fa fa-fw fa-plus"></i></a> |
...
<input class="btn btn-danger" type="button" onclick="tableToExcel('exporter', 'name', 'list.xls')" value="Export to Excel">
