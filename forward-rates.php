...
if ($conn->connect_error) {
  ...
  $cst_id = $_SESSION["userid"];
  $query_subs = "SELECT `status` FROM `subscription` WHERE `customer_id` = '$cst_id' and `status` = 'AVAILABLE'";
  $result_rows = $conn->query($query_subs);
  $subRowss = $result_rows->fetch_row();
...
?>
  ...
  <head>
    ...
    <link rel="stylesheet" href="./ratealert.css">
...
font-size: 15px !important;
  ...
  <body onload="rcvData();">
    ...
    <?php 
      if (isset($cst_id)) { 
        if ($subRowss['0'] === "AVAILABLE") { ?>
    <div class="rate-btn-area text-center">
          <button class="rate-subscribe-btn">Rate Alert</button>
    </div>
    <?php } }  ?>
    <!--<div class="container">
...
<!-- Rate alert modal from -->
<?php include_once("./ratealertmodal.php"); ?>

<!-- subscribe popup (tanmoy) -->

  <script>
    $(document).ready(function() {
      const baseUrl = "<?php echo $base_url; ?>";
      let subscription_checking = "<?= $allowFC; ?>";
      if(subscription_checking == "NO") { 
        var siturl = baseUrl+"/forward_rates_new";
        window.location = siturl;
      }

        $(".rate-subscribe-btn").click(function() {
            $("#alert-frm").addClass("show-alrt-pp");
            $(".frm-pop-bg").addClass("show-alrt-pp-bg");
            $(".frm-pop-bg").removeClass("close-alrt-pp");
            $("#alert-frm").removeClass("close-alrt-pp");
        });
        $(".frm-pop-bg").click(function() {
            $("#alert-frm").addClass("close-alrt-pp");
            $(this).addClass("close-alrt-pp");
        });
        $(".alrt-close-icon").click(function() {
            $("#alert-frm").addClass("close-alrt-pp");
            $(".frm-pop-bg").addClass("close-alrt-pp");
        });


        $(".rate-sv-btn").click(function(event) {
            event.preventDefault()
            
            var validationErrorcurrency = 0;
            var validationErrorRateType = 0;
            var validationErrorOperator = 0;
            var validationErrorTargetRate = 0;
            var validationErrorMobileNumber = 0;

            let currency = $("#currency").val();
            let rate_type = $("#rate_type").val();
            let operator = $("#operator").val();
            let target_rate = $("#target_rate").val();
            let mobile_number = $("#mobile_number").val();

            if(currency){
              validationErrorcurrency ? validationErrorcurrency-- : "";
              $("#currency-error-msg").html(' ');
            }else{
              validationErrorcurrency++;
              $("#currency-error-msg").html('Please Enter Currency');
            }
            if(rate_type){
              validationErrorRateType ? validationErrorRateType-- : "";
              $("#rate-type-error-msg").html(' ');
            } else{
              validationErrorRateType++;
              $("#rate-type-error-msg").html('Please Enter Rate Type');
            }
            if(operator){
              validationErrorOperator ? validationErrorOperator-- : "";
              $("#operator-error-msg").html(' ');
            }else{
              validationErrorOperator++;
              $("#operator-error-msg").html('Please Enter Operator');
            }
            if(target_rate){
              validationErrorTargetRate ? validationErrorTargetRate-- : "";
              $("#target-rate-error-msg").html(' ');
            } else{
              validationErrorTargetRate++;
              $("#target-rate-error-msg").html('Please Enter Target Rate');
            }
            if(mobile_number){
              validationErrorMobileNumber ? validationErrorMobileNumber-- : "";
              $("#mobile-number-error-msg").html(' ');
            }else {
              validationErrorMobileNumber++;
              $("#mobile-number-error-msg").html('Please Enter Mobile Number');
            }
            if(mobile_number){
              let mobile_number_length = mobile_number.length;
              if(mobile_number_length <= 9 ){
                validationErrorMobileNumber++;
                $("#mobile-number-error-msg").html('Please Enter valid 10 digit Mobile Number');
            ...
            if(validationErrorcurrency === 0 && validationErrorRateType === 0 && validationErrorOperator === 0 && validationErrorTargetRate === 0 && validationErrorMobileNumber === 0){
              $("#form-submit-msg").html('Please Wait').addClass('frm-success-msg');
              // var baseUrl = "<?php echo $base_url; ?>";
              var siturl = baseUrl+"/ajax-forward-rates";
              let formData = {"currency" : currency, "rate_type" : rate_type, "operator" : operator, "target_rate" : target_rate, "mobile" : mobile_number};
              $.ajax({
                        type: "POST",
                        url: siturl,
                        data: formData,
                        success: function (data) {                        
                          $("#form-submit-msg").html(' ').removeClass('frm-success-msg');

                          // console.log("response data settt =====> ", data);

                            if(data == '200'){
                              $("#form-submit-msg").removeClass('frm-error-msg');
                              $("#form-submit-msg").html('Successfully Submitted').addClass('frm-success-msg');
                              setTimeout(() => {
                                location.reload(true);
                              }, 2000);
                            } else if(data == '400'){
                              $("#form-submit-msg").removeClass('frm-error-msg');
                              $("#form-submit-msg").html('Already you added to rate alret').addClass('frm-error-msg');
                              setTimeout(() => {
                                location.reload(true);
                              }, 2000);
                            }else{
                              $("#form-submit-msg").removeClass('frm-success-msg');
                              $("#form-submit-msg").html('Some Thing went wrong').addClass('frm-error-msg');
                            }                        
                        }
                });
            }
